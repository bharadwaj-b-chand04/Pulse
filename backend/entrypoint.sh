#!/bin/sh
# Backend container entrypoint: migrate -> seed -> exec the CMD.
#
...
# harmlessly. The committed dataset is bind-mounted at /seed/data.
set -e

echo "[entrypoint] alembic upgrade head"
uv run alembic upgrade head

echo "[entrypoint] seed loader (identity + clinical entries)"
